...
<html><body>
  <div class="event-card event-upcoming">
    <h3>MVPW 07 - Turhan vs Baumgardner</h3>
    <h2>SUNDAY - NOVEMBER 8, 2026</h2>
    <a href="/event/mvpw-07-turhan-vs-baumgardner/">See Event Info</a>
  </div>
</body></html>
"""
...
def test_mvp_parser_retains_unique_event_url_from_event_card():
    events = parse_mvp_schedule(EVENT_CARD_HTML, date(2026, 9, 1))

    assert len(events) == 1
    assert events[0].title == "Turhan vs Baumgardner"
    assert events[0].source_url == (
        "https://www.mostvaluablepromotions.com/"
        "event/mvpw-07-turhan-vs-baumgardner/"
    )
